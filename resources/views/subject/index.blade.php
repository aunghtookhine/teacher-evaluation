...
@section('content')
    ...
    <div class="row justify-content-between my-3">
        <div class="col-md-3">
            <a href="{{ route('subject.create') }}" class="btn btn-color">Create</a>
        </div>
        <div class="col-md-5">
            <form action=" {{ route('subject.index') }}" method="get">
                <div class="input-group">
                    ...
                    @if (request()->has('keyword'))
                        <a href="{{ route('subject.index') }}" class="btn btn-color"><i class="bi bi-x"></i></a>
                    @endif
        ...
        <tbody>
            @forelse ($subjects as $subject)
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            {{-- <a href="{{ route('subject.show', $subject->id) }}" class="btn btn-sm btn-outline-color">
                                <i class=" bi bi-info"></i>
                            </a> --}}
                            <a href="{{ route('subject.edit', $subject->id) }}" class="btn btn-sm btn-outline-color">
                                <i class=" bi bi-pencil"></i>
                            </a>
                            <button form="subjectDeleForm{{ $subject->id }}" class=" btn btn-sm btn-outline-color">
            ...
            @empty
                <tr>
                    <td colspan="7" class=" text-center p-3">
                        ...
                        <a class="btn btn-sm btn-color" href="{{ route('subject.create') }}">Create
                            Subject</a>
                    </td>
